...
class Config:
    ...
    CODEBASE_ROOT = os.getenv("CODEBASE_ROOT", "../ProjectBase/CodeBase/")
    ...
    def validate_config(cls) -> List[str]:
        ...
        if not cls.AZURE_OPENAI_API_KEY:
            errors.append(
                "AZURE_OPENAI_API_KEY is required but not set. "
                "Please set it as an environment variable or create a .env file. "
                "See environment_setup.md for details."
            )

        if not cls.AZURE_OPENAI_ENDPOINT:
            errors.append("AZURE_OPENAI_ENDPOINT is required but not set")
